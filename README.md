...
Most friend groups split coordination across multiple apps: one for scheduling, another for chat, another for photos. Friendgroup consolidates event planning, event-scoped chat, and notifications into a single app, with group membership and permission controls baked in.
...
- **Notifications** — Web push (VAPID) and email (Resend). Per-user delivery preferences by type and tag.
...
- **User profiles** — Display name, avatar (S3-uploaded), and unique username (changeable once per year).
...
Edit `apps/api/.env` and set `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, and optionally `RESEND_API_KEY`. All other defaults work as-is for local development.
